...
const tiendas = [
  { id: 'tienda1', nombre: 'Carnicería Central' },
  { id: 'tienda2', nombre: 'Charcutería Norte' },
  // ...más tiendas
];
...
function App() {
  ...
  // Guardar pedidos en localStorage cada vez que cambian
  useEffect(() => {
    localStorage.setItem('pedidos', JSON.stringify(pedidos));
  }, [pedidos]);

  const cambiarEstadoPedido = (idx, nuevoEstado) => {
    setPedidos(pedidos =>
      pedidos.map((p, i) =>
        i === idx
          ? {
  ...
  const agregarPedido = (pedido) => {
    setPedidos([...pedidos, { ...pedido, tiendaId: tiendaSeleccionada, estado: 'borrador' }]);
  };

  // Nueva función para modificar un pedido
  ...
  // Nueva función para borrar un pedido
  const borrarPedido = (idx) => {
    setPedidos(pedidos => pedidos.filter((_, i) => i !== idx));
  };

  // Nueva función para enviar todos los pedidos de la tienda a fábrica
  const enviarPedidosAFabrica = () => {
    ...
    setPedidos(prevPedidos => {
      const actualizados = prevPedidos.map(p =>
        p.tiendaId === tiendaSeleccionada && p.estado === 'borrador'
          ? { ...p, estado: 'enviado', fechaPedido, numeroPedido: nuevoNumero }
          : p
      );
      // Guardar inmediatamente en localStorage para evitar asincronía
      localStorage.setItem('pedidos', JSON.stringify(actualizados));
      return actualizados;
    });
  };

  const abrirHistoricoEnVentana = (pedidos, tiendaId) => {
      ...
      <html>
        ...
        <body>
          ...
          <script>
            ...
            // Agrupa líneas por numeroPedido y tiendaId
            function agruparPorPedido(lista, fechaCampo) {
              ...
              // Devuelve array de objetos {numeroPedido, tiendaId, fecha, lineas}
            ...
            function renderTablas(desde, hasta) {
              // Realizados agrupados por numeroPedido y tiendaId
              ...
              // Recibidos agrupados por numeroPedido y tiendaId
  ...
  return (
    <div className="App">
      {!logueado ? (
        <Login onLogin={handleLogin} setModo={setModo} />
      ) : modo === 'fabrica' ? (
        <FabricaPanel pedidos={pedidos} cambiarEstadoPedido={cambiarEstadoPedido} />
      ) : (
        <div>
